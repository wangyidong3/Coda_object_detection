...
def is_intersect(min_x,min_y,max_x,max_y):
        
        centre_min_y, centre_min_x, centre_max_y, centre_max_x = (60,64,65,66)
        #print()
        if min_x > centre_max_x or max_x < centre_min_x:
            return False
        if min_y > centre_max_y or max_y < centre_min_y:
            return False
        return True
...
def draw_boxes(image, out_scores, out_boxes, out_classes, class_names, colors):
    ...
    for i, c in reversed(list(enumerate(out_classes))):
        ...
        #cv2.rectangle(image, (60, 64), (65, 329), tuple(reversed(colors[c])), 6)

        is_dangerous = is_intersect(left, top, right, bottom)
        ...
        cv2.rectangle(image, (60,64),(65,66),tuple(reversed(colors[c])), -1)
